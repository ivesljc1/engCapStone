...
def record_answer():
    ...
    if not all([questionnaire_id, user_id, question_id, answer]):
        return (
            jsonify(
                {
                    "error": "questionnaire_id, user_id, question_id, and answer are required"
                }
            ),
            400,
        )

    success, message = record_answer_to_question(
        questionnaire_id, user_id, question_id, answer
    )
...
@questionnaire_blueprint.route(
    "/api/questionnaire/get-most-recent-result", methods=["GET"]
)
...
# Get a questionnaire result by visitId
@questionnaire_blueprint.route(
    "/api/questionnaire/get-questionnaire-by-visit", methods=["GET"]
)
def api_get_questionnaire_by_visit():
    # Get the result_id from the query parameters
    ...
    result = get_result_by_visit(visit_id)
